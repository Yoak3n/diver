...
陪伴 UI，agent 大脑常驻�?Node sidecar（自�?cos harness，借鉴 DeepSeek Harness）�?

借鉴 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 的架构理�?
（append-only 会话日志、turn/step agent 循环、工具注册表、persona 组装、事件驱动）�?
**只取其框架、不搬其交互�?*：UI 为自�?Vue 单会话陪伴聊天，传输层为自研 HTTP/SSE�?
...
┌──────────────────────────────────────────────────�?
�?Tauri 壳（Rust�?                                 �?
�? · sidecar 生命周期管理（spawn/就绪检�?日志/重启�?�?
...
| 进程 | 端口 | 说明 |
|---|---|---|
| Node sidecar（dsh�?| `DIVER_PORT`（默�?**53620**�?| HTTP/SSE 服务；`DIVER_READY` 标志行报告就�?|
| Vite dev server | **1420**（strictPort�?| �?dev 构建；`/api` 代理�?53620 |
| Rust 本地服务（axum�?| 随机端口（绑�?127.0.0.1:0�?| 端口通过 `DIVER_MEMORY_PORT` 环境变量注入 sidecar |
    ...
    2. services::start() �?axum /rpc（SQLite 记忆后端），
     ...
     3. plugins::ensure_profile() + SidecarManager::start()
        ...
        （统一契约�?-profile companion --plugin-root cos-plugins
          ...
          COS_HOME=harness/.cos-home；检�?DIVER_READY�?
     4. �?window_startup 配置打开主窗�?/ 桌宠窗口
```

退出时（`RunEvent::Exit`）主�?`stop()` sidecar；主窗口关闭只隐藏不退�?
（`CloseRequested` �?`prevent_close` + hide），agent 持续运行�?

退出清理三级兜底（`core/sidecar/` `stop()`）：
1. `POST /api/shutdown`（令牌校验）�?Node �?`settle()` 优雅 dispose agent 树后 exit
2. 超时/失败 �?`Child::kill()` 强制终止
3. Windows Job Object（KILL_ON_JOB_CLOSE）→ 应用退�?被强杀时整进程树兜底清�?

详见 [Node sidecar �?cos 接入](sidecar.md)、[插件体系与生命周期](plugins.md)�?

## 窗口管理

`src-tauri/src/shell/window/` 统一管理窗口生命周期�?

- **WindowType**：`Main`（主聊天窗口�? `Pet`（Live2D 桌宠，透明/置顶/无边�?跳过任务栏）
- **Manager**：`show_window` / `close_window` / `destroy_window` / `toggle_window`�?
  维护 `WindowState` 缓存，避免重复创�?
- **轻量模式**（`shell/lightweight.rs`）：窗口关闭计时器触发，隐藏窗口�?sidecar �?
  桌宠继续运行，到点恢复主窗口
...
- **单实�?*：第二个实例启动时经命名管道通知已有实例（回调把主窗口带到前台），自身退�?

## 本地服务（services/�?

Tauri 侧用 axum 起一个只监听 `127.0.0.1` �?HTTP 服务，供 Node sidecar 调用
（sidecar 作为 agent 进程不直接持�?SQLite 连接）：

- `POST /rpc`：统一 JSON-RPC 入口（`{ method, params }` �?`{ ok, data }` / `{ ok: false, error }`�?
- 当前路由：`grep::*` 前缀 �?`grep::dispatch`（grep 搜索，`spawn_blocking` �?
  `diver-search` 引擎）；其余 �?`memory::dispatch`（见 [关系层记忆](memory.md)�?
- 扩展方式：`ServiceState` 加字�?�?`rpc::route` �?method 前缀分流 �?merge �?Router

grep 搜索后端（crates/diver-search）：�?`grep-regex` / `grep-searcher` / `ignore`
库进程内嵌入 ripgrep 引擎（与 rg 同源目录语义），免去打包 rg.exe 二进制的负担�?
Node �?`@diver/basic-tools` �?grep 工具经同一 `/rpc` 通道调用
`grep::search`（参�?`pattern` / `path` / `include?` / `maxMatches?` /
`maxBytesPerLine?`），错误带稳�?code（`INVALID_PATTERN` / `INVALID_TARGET` 等）�?
...
| command | 说明 |
|---|---|
| `get_sidecar_status` / `restart_sidecar` / `get_sidecar_url` | sidecar 状�?/ 重启 / UI 地址 |
...
| turn/step agent 循环 | `@cos/agent-loop`：流�?chunk、工具闭环等 |
| 工具注册�?+ 执行管线 | `@cos/tools`；能力面�?`@diver/*` 插件注册 |
| persona 组装 | bundle / profile patch 覆盖 `system-prompt` �?|
| 一切皆�?patch / 可插�?| profile + bundle + 壳端启停（[plugins.md](plugins.md)�?|
| 事件驱动 UI | session/event + agent/* �?SSE �?Vue |

## 前端通信

`src/api.ts` 封装 sidecar HTTP API（`/api/health`、`/api/chat`、`/api/settings`�?
`/api/question-answer`），`streamEvents` �?EventSource/SSE 消费流式事件�?
