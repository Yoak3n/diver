...
- **Node 插件**（`cos-plugins/memory`）— agent 侧的智能层：
  agent 主动 remember / entity 工具、身份卡片（identity）工具、会话末 digest 归纳、压缩摘要内化、常驻注入、agent 工具面
- **Rust 后端**（`crates/diver-memory` + `src-tauri/src/services/`）— 确定性层：
  SQLite 存储、衰减/激活/遗忘、阻塞候选、实体图谱、统计快照，经本地 HTTP JSON-RPC 暴露
...
agent（Node）
  ...
  ├─ 主动 entity 工具：显式声明实体/属性/关系 → 知识图谱（无规则抽取）
  ├─ 身份卡片 identity 工具：agent 主动完善"我是什么样的人"（性格/说话方式/价值观）
  ├─ 会话末 digest（节流）：transcript → LLM → 身份卡片增量 + 图谱补全
  ├─ compaction/summary：把压缩摘要内化为长期记忆
  ├─ 写入：remember / entity / append_event / upsert_promise / update_card（经 RPC）
  └─ 常驻注入：systemPrompt.section(order 15) ← 同步视图缓存（快照）
          │
          ▼ HTTP JSON-RPC（127.0.0.1:<DIVER_MEMORY_PORT>/rpc）
Rust（axum + rusqlite，单文件 SQLite）
  ├─ topics / events / relation_card / promises / self_history
  ├─ entities / entity_attrs / entity_relations（知识图谱，挂在 topics 旁）
  ├─ 衰减/激活/遗忘（读取相关数据时懒执行）
  ├─ blocking_candidates / entity_candidates / stats / snapshot
...
-- 实体图谱（方案 B：schema 参考 mneme-rs，写入走 agent/digest 显式声明）
entities        -- 实体节点：name 唯一/entity_type/mention_count
entity_attrs    -- 时序属性：同 key 旧值封存 valid_until，source_topic_id 溯源
entity_relations-- 有向关系：from→to/relation，同三元组幂等
```

## 实体图谱（知识图谱层）

设计取舍：**只借 mneme-rs 的图谱 schema，不移植其规则抽取/共现边**。
写入唯一来源是 agent / digest 显式声明，避免 CJK 词段噪声稀释 recall。

| 项 | 约定 |
|---|---|
| 写入工具 | `entity`：name + 可选 type/attrs/relations；关系目标不存在则自动创建 |
| 合并语义 | 同名实体 upsert 加强；属性时序覆盖；关系同 (from,to,relation) 幂等 |
| 读取 | `recall` / `inventory` 附带 `entities`：命中实体 + 一跳属性与关系 |
| 与 topics 关系 | 并列存储；`source_topic_id` 可选溯源，不替换双层记忆模型 |
| 不做 | 逐轮规则抽取、共现 `related_to` 自动建边、agent 亲手删图（只写入/加强） |
...
| 实体图谱 | `upsert_entity` `set_entity_attr` `add_entity_relation` `upsert_entity_graph` |
| 实体读取 | `find_entity` `entity_graph` `entity_candidates` `list_entities` `search_entities` |
| 诊断 | `stats` `snapshot` |
...
| Agent 主动 entity（工具） | 显式声明实体/属性/关系写入图谱；**不做规则抽取** |
| 身份卡片 identity（工具） | agent 对"我是什么样的人"有了稳定看法时主动调用 `identity` 写 `agent_model`/`relationship`，让性格跨会话保持、持续完善 |
| 会话末 digest（节流） | 积累 ≥3 对 turn pair 且距上次 digest ≥10 分钟 → LLM 归纳身份卡片增量 + 图谱补全 |
...
| Agent 工具面 | `remember` / `entity` / `recall` / `inventory` / `demote` / `identity`（只加强/减弱，不亲手删，可逆） |
| 快照视图 | store-rpc 维护 `snapshot` 缓存：card/promises 全量，topics/events 只取最近 200 条；`events` 超出 2000 条自动裁剪 |
| 读取 | `recall` 走 `blocking_candidates`（词法 contains）+ `entity_candidates`（实体名命中 → 一跳属性/关系）；无命中时 Mode B 近期经历兜底 |

## 验证

`cargo test -p diver-memory`：实体图谱写入 / 属性时序 / 关系幂等 / 候选召回单测。

`node scripts/memory-test.mjs` 覆盖：引导 agent 通过 `remember` 工具写入事实 → recall 型问题
（名字/忌口/爱好）回答正确 → 调 `/rpc` 做 create/snapshot/delete 与实体图谱读写的 SQLite 落库验证。
注意：**不再断言"每轮自动 remember"**；记忆来源为 ① agent 主动 `remember` / `entity` ② 会话末 digest ③ 压缩摘要内化。
