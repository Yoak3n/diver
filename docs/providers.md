...
| 提供商 | 声明来源 | 声明字段 | 说明 |
|---|---|---|---|
| `deepseek-official` | `@cos/llm-deepseek` 的 `providerConfig()` | apiKey（password/credentials，必填，env 兜底 `DEEPSEEK_API_KEY`）+ baseUrl（text/settings，可自定义端点） | 官方 API（`deepseek-flash` / `deepseek-pro`；第三方提供商处同一模型保留全名 `deepseek-v4.1-flash`） |
