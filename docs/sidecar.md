# Node sidecar �?cos harness 接入

agent 大脑是一�?*常驻 Node 进程**，运行仓库内自研 **cos harness**（`harness/`，`@cos/*`）�?
陪伴插件 `@diver/*` 源码�?`cos-plugins/`（dev）或安装�?`plugins/`（release），
�?*统一 loader 契约**挂载；启停状态由壳写�?profile 补丁层�?

> **插件组合、启停、壳端管理、深水区分期的权威文档见 [plugins.md](plugins.md)�?*
> 本页只保�?sidecar 进程生命周期�?harness 能力接入摘要�?

## 角色与生命周�?

- sidecar �?Rust `core/sidecar/` 启动�?*dev / release 同一契约**�?

  | 参数 | dev | release |
  |---|---|---|
  | 入口 | `companion.ts` | `companion-bundle.ts` |
  | Node | 系统 `node` + `tsx` | 本机/缓存 Node �?22（`node_runtime` 解析或下载）+ tsx loader |
...
- 环境变量：`DIVER_PORT`（默�?53620）、`DIVER_MEMORY_PORT`、`DIVER_SHUTDOWN_TOKEN`�?
  `DIVER_UI_DIST`、`DIVER_MCP_CONFIG_FILE`
- stdout 检�?`DIVER_READY` �?状�?Running；主窗口隐藏不影�?sidecar
...
**SEA 已退�?release 路径**（开放插件目�?+ 首启解析/下载 Node�?*不随�?node.exe**）�?
...
| tools / systemPrompt / credentials / llm | `@cos/*` 核心�?|
| persona / 传输�?| `@diver/*` 插件（voice / backend�?|
| Web UI 交互�?| **不用**，自�?Vue + `@diver/backend` HTTP/SSE |
...
$COS_HOME/profiles/companion/
  package.json          # dsh.profile.bundles（diver 默认空，bundle �?CLI 显式传入�?
...
- 压缩：`compaction-basic` 阈值摘要；`compaction/summary` 事件�?`@diver/memory` 内化

## 相关文档

- [plugins.md](plugins.md) �?插件契约与生命周�?
- [architecture.md](architecture.md) �?三层架构与启动时�?
- [development.md](development.md) �?独立调试 sidecar
- [distribution.md](distribution.md) �?不随�?Node 的分发与首启运行时布局
