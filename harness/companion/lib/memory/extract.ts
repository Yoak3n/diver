...
async function llmText(
  ...
  const request = {
    provider: diver.provider ?? selection.provider ?? 'deepseek-official',
    model: diver.model ?? selection.model ?? 'deepseek-v4-flash',
    system: options.system,
    messages: [
      createUserMessage({ content: [{ type: 'text', text: options.prompt }], source: SOURCE }),
    ],
  ...
  try {
    ...
    console.error(`[memory] LLM 调用失败: ${err?.message ?? err}`)
    return null
  }
  return text || null
}
