...
//!
//! 依赖方向：services → core/config/crates；禁止 use `crate::app` / `crate::shell`
//! （壳能力经 start 时注入的闭包提供）。
...
pub fn start(app: &AppHandle, notify: NotifyFn) -> Option<u16> {
    ...
    let state = ServiceState { memory_db, notify };
