// 聊天核心状态与逻辑（消息流、SSE 事件、发送、TTS、连接生命周期）
...
export function useChat() {
  ...
  const canSend = computed(() =>
    !!healthInfo.value?.ok && modelConfigured.value && !busy.value && !connecting.value,
  );
      ...
      case "message":
        if (e.kind === "user") {
          const dup = messages.value.some(
            (m) => m.kind === "user" && m.content === e.content && Date.now() - m.time < 10000,
          );
